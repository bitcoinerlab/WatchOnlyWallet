...
export default function Home() {
  ...
  const fetch = useCallback(async () => {
    console.log("TRACE: fetching", { discovery: !!discovery });
    if (discovery) {
      console.log("TRACE: fetching start...");
      setFetching(true);
  ...
  const isFetched = discovery?.whenFetched({ descriptor }) || false;
  const history = (
  ...
  return (
    <ScrollView>
      <View className="flex-1 justify-center items-center bg-gray-100 p-4">
        ...
        {discovery ? (
          <Text className="text-red-500 mt-4" onPress={disconnect}>
            Disconnect
          </Text>
        ) : (
          <Text className="text-blue-500 mt-4" onPress={connect}>
            Connect
          </Text>
        )}
        ...
        {fetching ? (
          ...
          <Text
            className={`${discovery ? "text-blue-500" : "text-gray-400"} mt-4`}
            {...(discovery ? { onPress: fetch } : {})}
          >
            Fetch
          </Text>
        )}
